var AthleteStatsModifier = (function () {
    ...
    AthleteStatsModifier.prototype.init = function (activities) {
        var _this = this;
        ...
        this.progressThisYear.find("#athleteStatsLoadingForceRefresh").show().click(function (e) {
            e.preventDefault();
            _this.handleProgressStatsForceRefresh();
        });
        ...
        this.progressThisYear.find('#stravistix_yearProgress_incVirtualRides').on('click', function () {
    ...
    AthleteStatsModifier.prototype.formatData = function (activities) {
    ...
    AthleteStatsModifier.prototype.renderTrendArrow = function (value, formatFunction) {
        if (value === undefined) {
            return "";
        }
    ...
    AthleteStatsModifier.prototype.processData = function (activities, progressThisYear, distanceInKilometers, elevationInMeters, distanceUnit) {
        var types = [[], []];
        var years = [];
        var yearsList = [];
        var i;
        ...
        var _loop_1 = function(i_1) {
            years = types[i_1];
            yearsList = [];
            var $table = $("<table class='athletesStatTable' id='athletesStatActivityType" + i_1 + "' style='display: none;'><thead><tr><th>Year</th><th>Distance</th><th>Elevation</th><th>Time</th></tr></thead><tbody></tbody></table>");
            ...
            for (j = 0, max = yearsList.length; j < max; j++) {
                ...
                $table.find("tbody").append($("<tr class='" + (isCurrentYear ? 'currentyear' : '') + "'>" +
                    ...
                    "<td><div style='white-space: nowrap;'>" + Helper.formatNumber(item.distance, 0) + " " + distanceUnit + this_1.renderTrendArrow(distanceDifference, function (value) {
                    return Helper.formatNumber(Math.abs(value), 0) + " " + distanceUnit;
                }) + "</div><div style='white-space: nowrap;'><small>" + Helper.formatNumber(item.count, 0) + " " + (i_1 == 0 ? "Rides" : "Runs") + this_1.renderTrendArrow(activitiesCountDifference, function (value) {
                    return Helper.formatNumber(Math.abs(value), 0) + " " + (i_1 == 0 ? "Rides" : "Runs");
                }) + "</small></div></td>" +
                    "<td><div style='white-space: nowrap;'>" + Helper.formatNumber(item.elevation, 0) + " " + this_1.elevationUnit + this_1.renderTrendArrow(elevationDifference, function (value) {
                    return Helper.formatNumber(Math.abs(value), 0) + " " + this.elevationUnit;
                }) + "</div></td>" +
                    "<td><div style='white-space: nowrap;'>" + Helper.secondsToDHM(item.time) + this_1.renderTrendArrow(timeDifference, function (value) {
                    return Helper.secondsToDHM(Math.abs(value));
                }) + "</div></td>" +
                    "</tr>"));
            }
            progressThisYear.append($table);
            progressThisYear.find("a[data-activity-type=" + i_1 + "]").show();
    ...
    ;
    AthleteStatsModifier.prototype.modify = function () {
        var _this = this;
        var self = this;
        ...
        var total = 0, i, max, url = '/athlete/training_activities?new_activity_only=false&per_page=2000&page=', currentActivities = [], requests = [], activitiesFromCache = localStorage.getItem(this.cacheKey_), activitiesFromCacheObject = JSON.parse(activitiesFromCache) || [], progress = $("#progress-goals-v2");
        ...
        $(this.progressThisYear).on("click", "a[data-activity-type]", function (e) {
            e.preventDefault();
            var $this = $(this), activityType = $this.data("activity-type");
        ...
        $(this.progressThisYear).on("click", "#athleteStatsShowChart a", function (e) {
            e.preventDefault();
            var activityType = self.progressThisYear.find("a[data-activity-type].selected").data("activity-type");
            var size = [
        ...
        var prepareChart = function (activities) {
            var i, j, numberOfDays = 366, max, activity, day, currentTargetType, currentDataType = 1, data = [], minValue = 0, maxValue = 0, leapYear = 2000, firstDayDate = new Date(leapYear, 0, 1), lastDayDate = new Date(leapYear, 11, 31, 23, 59, 59), currentDate = new Date(), currentYear = currentDate.getFullYear(), oneDayInMiliseconds = 1000 * 60 * 60 * 24, dayOfYear = function (date) {
                ...
                return result;
            }, currentDayOfYear = dayOfYear(currentDate), formatValue = function (value) {
            ...
            processData();
            var container = "#athleteStatChart", width = $(container).width(), height = $(container).height();
            var margin = {
                top: 20,
                right: 80,
                bottom: 30,
                left: 90
            }, w = width - margin.left - margin.right, h = height - margin.top - margin.bottom;
            ...
            var color = d3.scale.category10(), trendLinesGroup = svg.append("svg:g");
            var generateLines = function () {
                ...
                data.forEach(function (yearData) {
                    ...
                    var year = yearData.year, id = "ascy" + year, liYear = $("<li style='margin: 8px'><input id='" + id + "' checked type='checkbox' value='" + year + "'/> <label for='" + id + "' style='display: inline; color: " + color(i.toString()) + ";'>" + yearIdentifier + "</label></li>"), liSpan = $("<span style='display: inline-block; margin-left: 10px; width: 80px; text-align: right; color: black;'></span>");
            ...
            var handleMouseOverGraph = function (event) {
                var mouseX = event.pageX - hoverLineXOffset, mouseY = event.pageY - hoverLineYOffset;
                if (mouseX >= 0 && mouseX <= w && mouseY >= 0 && mouseY <= h) {
                    hoverLine.attr("x1", mouseX).attr("x2", mouseX);
                    hoverLineText.attr("x", mouseX + 5);
                    var date = x.invert(mouseX), day_1 = dayOfYear(date);
                    data.forEach(function (item) {
                        if (day_1 < item.values.length && !item.target) {
                            item.$value.text(formatValue(item.values[day_1]));
                        }
                        else {
                            item.$value.text("");
                            if (item.target) {
                                switch (currentDataType) {
                                    case 1:
                                    case 3:
                                        var showValue = (item.values[1] - item.values[0]) / numberOfDays * day_1;
            ...
            requests.push($.ajax({
                ...
                success: function (data) {
                    ...
                    $.when.apply(self, requests).done(function () {
                        ...
                        currentActivities = _this.formatData(currentActivities);
    ...
    AthleteStatsModifier.prototype.handleProgressStatsForceRefresh = function () {
        localStorage.removeItem(this.cacheKey_);
        window.location.reload();
    };
    AthleteStatsModifier.metersTo1000thOfMileFactor = 0.621371192;
    AthleteStatsModifier.metersToFeetsFactor = 3.2808399;
    return AthleteStatsModifier;
}());
